...
전체 FPGA에는 50개의 블록이 있으며, 각 블록은 36Kb(킬로비트)의 용량을 가지고 있다. 이는 대량의 데이터를 저장할 수 있는 큰 용량의 메모리다. 또한 전용 메모리의 일부(약 25-50%)는 시프트 레지스터로 구성될 수도 있다. 이는 데이터 이동이나 변환 작업과 같은 특정 용도에 적합한 기능을 제공한다.
